import React, { useState, useEffect } from "react";
import { Map, TileLayer, Marker, Popup } from "react-leaflet";
import L from "leaflet";
import "leaflet/dist/leaflet.css";
import Geocode from "react-geocode";
import axios from "axios";
import apiAddress from "../global/endpointAddress";

const OnePagerMap = (props) => {
  const [lat, setLat] = useState(0);
  const [long, setLong] = useState(0);
  console.log(props);

  useEffect(() => {
    if (
      props.location &&
      !(props.location.Latitude === 0 && props.location.Longitude === 0)
    ) {
      setLat(props.location.Latitude);
      setLong(props.location.Longitude);
    } else {
      Geocode.setApiKey("AIzaSyDKrJKYOJ507vkFJQgLfPO_PZvA9Noox6U");
      Geocode.setLanguage("en");
      const getCoordinates = async (city, address) => {
        const response = await Geocode.fromAddress(address + " ," + city).then(
          (res) => {
            setLat(res.results[0].geometry.location.lat);
            setLong(res.results[0].geometry.location.lng);
            return res;
          },
          async (error) => {
            console.log("ERROR");
            const response = await Geocode.fromAddress(city);
            return response;
          }
        );
        return response;
      };

      const asignCoordinates = async () => {
        const resData = await getCoordinates(props.city, props.address);
        axios
          .post(
            `${apiAddress}/api/SmallCompanies/SetCompanyCoordinates?companyID=${props.companyID}&lat=${resData.results[0].geometry.location.lat}&lon=${resData.results[0].geometry.location.lng}`,
            {},
            {
              headers: {
                Authorization: "Basic " + sessionStorage.getItem("Token"),
                "Access-Control-Allow-Origin": "*",
              },
            }
          )
          .then((res) => {})
          .catch((e) => {
            console.log("Error with writing coordinates");
          });
      };
      asignCoordinates();
    }
  }, []);

  const position = [lat, long];
  const myIcon = L.icon({
    iconUrl:
      "data:image/png;base64,",
    iconSize: [25, 41],
    iconAnchor: [12.5, 41],
    popupAnchor: [0, -41],
  });

  return (
    <div className="onepager-map-main" data-html2canvas-ignore>
      <Map className="onepager-map" center={position} zoom={13}>
        <TileLayer
          attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />
        <Marker position={position} icon={myIcon}>
          <Popup>{props.address}</Popup>
        </Marker>
      </Map>
    </div>
  );
};

export default OnePagerMap;
